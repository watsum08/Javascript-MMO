...
export class Player {
    ...
    update(input: InputHandler, deltaTime: number): void {
        ...
        // Boundary checks
        if (this.worldX - this.radius < 0) this.worldX = this.radius;
        if (this.worldX + this.radius > MAP_WIDTH) this.worldX = MAP_WIDTH - this.radius;
        if (this.worldY - this.radius < 0) this.worldY = this.radius;
        if (this.worldY + this.radius > MAP_HEIGHT) this.worldY = MAP_HEIGHT - this.radius;
    }
}